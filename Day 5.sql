DECLARE @input varchar(max)='seeds: 3121711159 166491471 3942191905 154855415 3423756552 210503354 2714499581 312077252 1371898531 165242002 752983293 93023991 3321707304 21275084 949929163 233055973 3626585 170407229 395618482 226312891
...
DECLARE @part tinyint=1;

--- Part 1:

DROP TABLE IF EXISTS #recursion, #maps;

--- This is the anchor of the recursion, the seed numbers:
--- "map_number" is the ordinal of the mapping; 1 being seed-to-soil, 2 is soil-to-fertilizer, and so on.
SELECT CAST(ss.[value] AS bigint) AS seed_number, CAST(ss.[value] AS bigint) AS id, 0 AS map_number
INTO #recursion
FROM STRING_SPLIT(REPLACE(@input, CHAR(13), ''), CHAR(10), 1) AS source
CROSS APPLY STRING_SPLIT(SUBSTRING(source.[value], 8, LEN(source.[value])), ' ') AS ss
...
WITH source AS (
    ...
    FROM STRING_SPLIT(REPLACE(@input, CHAR(13), ''), CHAR(10), 1) AS i
    WHERE i.[value]!=''),

--- ... then use a gaps-and-islands pattern to derive the mapping table...
maps AS (
    SELECT sub.map_number,
           CAST(MAX((CASE WHEN ss.ordinal=1 THEN ss.[value] END)) AS bigint) AS dest_range_from,
           CAST(MAX((CASE WHEN ss.ordinal=2 THEN ss.[value] END)) AS bigint) AS src_range_from,
           CAST(MAX((CASE WHEN ss.ordinal=3 THEN ss.[value] END)) AS bigint) AS range_length
    FROM (
        SELECT ordinal, _map_increment,
               SUM(_map_increment) OVER (ORDER BY ordinal ROWS UNBOUNDED PRECEDING) AS map_number,
               [value]
        FROM (
            SELECT ordinal, (CASE WHEN [value] LIKE '%map:' THEN 1 ELSE 0 END) AS _map_increment, [value]
            FROM source
            WHERE ordinal>1
            ) AS sub
    ) AS sub
    CROSS APPLY STRING_SPLIT(sub.[value], ' ', 1) AS ss
    WHERE sub.[value] NOT LIKE '%map:'
    GROUP BY sub.ordinal, sub.map_number)

--- ... which we'll put in #maps:
--- map_number          tinyint: the ordinal of the mapping transformation.
--- dest_range_from     bigint
--- src_range_from      bigint
--- range_length        bigint
SELECT map_number, dest_range_from, src_range_from, range_length
INTO #maps
FROM maps;

CREATE UNIQUE CLUSTERED INDEX maps ON #maps (map_number, src_range_from);

--- Recursion happens here:
DECLARE @map_number tinyint=0;

--- Loop through map_numbers 1, 2, 3, ..., 6, 7.
WHILE (@map_number<=(SELECT MAX(map_number) FROM #maps)) BEGIN;
    INSERT INTO #recursion (seed_number, id, map_number)
    SELECT rec.seed_number, ISNULL(rec.id-maps.src_range_from+maps.dest_range_from, rec.id) AS id, rec.map_number+1
    FROM #recursion AS rec
    --- For each step, apply the mapping from #maps - if there is one -.
    LEFT JOIN #maps AS maps ON maps.map_number=rec.map_number+1 AND rec.id BETWEEN maps.src_range_from AND maps.src_range_from+maps.range_length-1
    WHERE rec.map_number=@map_number;

    SET @map_number=@map_number+1;
    --- Rinse, repeat.
END;

--- The lowest restulting mapped id (the location) is the output:
SELECT MIN(id) FROM #recursion WHERE map_number=@map_number-1;
...
--- Pad the gaps in the #maps table so it becomes a dense mapping table. This means
--- that _whatever_ value I want to map, there will be a range in the #maps table
--- that includes it.
INSERT INTO #maps (map_number, dest_range_from, src_range_from, range_length)
SELECT map_number, _src_range_from, _src_range_from, _range_length
FROM (
    SELECT map_number,
        LAG(src_range_from+range_length, 1, 0) OVER (PARTITION BY map_number ORDER BY src_range_from) AS _src_range_from,
        src_range_from-LAG(src_range_from+range_length, 1, 0) OVER (PARTITION BY map_number ORDER BY src_range_from) AS _range_length
    FROM #maps
) AS sub
WHERE _range_length>0;

--- Pad the end of #maps
--- Like the above step, but padding the table, so there's a match all the way
--- up to 1,000,000,000,000:
INSERT INTO #maps (map_number, dest_range_from, src_range_from, range_length)
SELECT map_number, MAX(src_range_from+range_length), MAX(src_range_from+range_length), POWER(CAST(10 AS bigint), 12)-MAX(src_range_from+range_length)
FROM #maps
GROUP BY map_number;



DROP TABLE IF EXISTS #recursion2;

--- The anchor of our new recursion consists of _ranges_ of values, rather than
--- individual values:
SELECT CAST(0 AS tinyint) AS map_number,
       --- The %2 thing is to assign every two values to seed_range_from and seed_range_to respectively.
       MAX((CASE WHEN (ss.ordinal+1)%2=0 THEN CAST(ss.[value] AS bigint) END)) AS seed_range_from,
       SUM(CAST(ss.[value] AS bigint))-1 AS seed_range_to,
       --- The starting value of the mapped_range is the same as seed_range, but mapped_range
       --- will evolve as we recurse:
       MAX((CASE WHEN (ss.ordinal+1)%2=0 THEN CAST(ss.[value] AS bigint) END)) AS mapped_range_from,
       SUM(CAST(ss.[value] AS bigint))-1 AS mapped_range_to
INTO #recursion2
FROM STRING_SPLIT(REPLACE(@input, CHAR(13), ''), CHAR(10), 1) AS source
CROSS APPLY STRING_SPLIT(SUBSTRING(source.[value], 8, LEN(source.[value])), ' ', 1) AS ss
WHERE source.ordinal=1
GROUP BY (ss.ordinal+1)/2;

SET @map_number=0;

--- Recursion starts here:
WHILE (@map_number<(SELECT MAX(map_number) FROM #maps)) BEGIN;
    INSERT INTO #recursion2 (map_number, seed_range_from, seed_range_to, mapped_range_from, mapped_range_to)
    SELECT rec.map_number+1 AS map_number,
           GREATEST(rec.mapped_range_from, maps.src_range_from) AS seed_range_from,
           LEAST(rec.mapped_range_to, maps.src_range_from+maps.range_length-1) AS seed_range_to,
           GREATEST(rec.mapped_range_from, maps.src_range_from)+o.offset AS mapped_range_from,
           LEAST(rec.mapped_range_to, maps.src_range_from+maps.range_length-1)+o.offset AS mapped_range_to
    FROM #recursion2 AS rec
    --- We're joining two (partially or fully) overlapping ranges (#recursion and #maps).
    --- If range 1 is (a->b) and range 2 is (c->d), we'll join rows where (a<=d and b>=c).
    --- This produces a cartesian product that splits the dataset into one row for each permutation of (a, c).
    --- So, for every recursion, we'll an equal or greater amount of rows than we did in the last one.
    INNER JOIN #maps AS maps ON
        rec.mapped_range_from<=maps.src_range_from+maps.range_length-1 AND
        rec.mapped_range_to>=maps.src_range_from AND
        rec.map_number+1=maps.map_number
    --- The "offset" here is the difference between the dest_range_from and src_range_from,
    --- which is effectively what we're adding to the incoming range boundaries to create
    --- the outgoing boundary values.
    CROSS APPLY (
        VALUES (maps.dest_range_from-maps.src_range_from)
        ) AS o(offset)
    WHERE rec.map_number=@map_number
    ORDER BY rec.seed_range_from, maps.src_range_from;

    SET @map_number=@map_number+1;
END;

--- The lowest mapped mapped_range_from of the last map_number is the output:
SELECT TOP (1) MIN(mapped_range_from)
FROM #recursion2
GROUP BY map_number
