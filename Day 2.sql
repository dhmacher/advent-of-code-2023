DECLARE @input varchar(max)='Game 1: 1 green, 2 red, 6 blue; 4 red, 1 green, 3 blue; 7 blue, 5 green; 6 blue, 2 red, 1 green
...
WITH parsed AS (
    SELECT game.game, round.ordinal AS [round], [cube].[count], [cube].color
    FROM STRING_SPLIT(REPLACE(@input, CHAR(13), ''), CHAR(10)) AS i
    CROSS APPLY (
        VALUES (CAST(REPLACE(LEFT(i.[value], CHARINDEX(': ', i.[value])-1), 'Game ', '') AS int),
                SUBSTRING(i.[value], CHARINDEX(': ', i.[value])+2, LEN(i.[value])))
        ) AS game(game, rounds)
    CROSS APPLY STRING_SPLIT(REPLACE(game.rounds, '; ', ';'), ';', 1) AS [round]
    CROSS APPLY STRING_SPLIT(REPLACE(round.[value], ', ', ','), ',') AS cubes
    CROSS APPLY (
        VALUES (CAST(LEFT(cubes.[value], CHARINDEX(' ', cubes.[value])-1) AS int),
                SUBSTRING(cubes.[value], CHARINDEX(' ', cubes.[value])+1, 10))
        ) AS [cube]([count], color))

SELECT SUM(game)
FROM (
    SELECT game, MAX((CASE WHEN color='red' AND [count]>12 THEN 1
                        WHEN color='green' AND [count]>13 THEN 1
                        WHEN color='blue' AND [count]>14 THEN 1
                        ELSE 0 END)) AS disqualified
    FROM parsed
    GROUP BY game) AS sub
...
FROM (
    SELECT game,
        MAX((CASE WHEN color='red' THEN [count] ELSE 0 END))*
        MAX((CASE WHEN color='green' THEN [count] ELSE 0 END))*
        MAX((CASE WHEN color='blue' THEN [count] ELSE 0 END)) AS pwr
    FROM parsed
    GROUP BY game
) AS x;
