DECLARE @input varchar(max)='.7F77F-J-F.LF-|-LL.J-|-F.7-|77L-L.FLFFJ7-7777-|7-F7.LFF--7F.LJ7-J7F-7-FL-7J77|-L-7-F-F--.J7..7FFLJ7..|7.F|--7FL|-|7F--J---JF-LL-L7-|.|-LJ7.F
...
DECLARE @width smallint=PATINDEX('%['+CHAR(13)+CHAR(10)+']%', @input)-1;
SET @input=REPLACE(REPLACE(@input, CHAR(10), ''), CHAR(13), '');
DECLARE @S bigint=CHARINDEX('S', @input);
...
WITH rcte AS (
    --- This is our starting point, the S:
    SELECT @S AS prev_offset,
           @S AS offset,
           CAST(0 AS tinyint) AS [path],
           0 AS step

    UNION ALL

    SELECT rcte.offset AS prev_offset,
           o.next_offset,
           (CASE WHEN rcte.[path]=0 THEN CAST(ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS tinyint) ELSE rcte.[path] END) AS [path],
           rcte.step+1
    FROM rcte
    --- These are the directions we could go from here:
    CROSS APPLY (
        --- North
        SELECT 'N' AS dir, rcte.offset-@width AS next_offset
        WHERE rcte.offset>@width
        UNION ALL

        --- East
        SELECT 'E' AS dir, rcte.offset+1 AS next_offset
        WHERE rcte.offset%@width<@width
        UNION ALL

        --- South
        SELECT 'S' AS dir, rcte.offset+@width AS next_offset
        WHERE rcte.offset+@width<LEN(@input)
        UNION ALL

        --- West
        SELECT 'W' AS dir, rcte.offset-1 AS next_offset
        WHERE (offset-1)%@width+1>0
    ) AS o
    --- ... which maps to these symbols:
    CROSS APPLY (
        VALUES (
            SUBSTRING(@input, rcte.offset, 1),
            SUBSTRING(@input, o.next_offset, 1)
        )
    ) AS x(this, [next])
    --- And these are the _valid_ directions we could go:
    CROSS APPLY (

        --- Valid connections if we're already on a path:
        SELECT o.next_offset WHERE o.dir='N' AND x.[this] IN ('|', 'L', 'J')
        UNION ALL
        SELECT o.next_offset WHERE o.dir='E' AND x.[this] IN ('-', 'L', 'F')
        UNION ALL
        SELECT o.next_offset WHERE o.dir='S' AND x.[this] IN ('|', '7', 'F')
        UNION ALL
        SELECT o.next_offset WHERE o.dir='W' AND x.[this] IN ('-', '7', 'J')

        UNION ALL

        --- But if we're at the S, our valid paths depend on the target symbols:
        SELECT o.next_offset WHERE x.[this]='S' AND o.dir='N' AND x.[next] IN ('|', '7', 'F')
        UNION ALL
        SELECT o.next_offset WHERE x.[this]='S' AND o.dir='E' AND x.[next] IN ('-', '7', 'J')
        UNION ALL
        SELECT o.next_offset WHERE x.[this]='S' AND o.dir='S' AND x.[next] IN ('|', 'L', 'J')
        UNION ALL
        SELECT o.next_offset WHERE x.[this]='S' AND o.dir='W' AND x.[next] IN ('-', 'L', 'F')

    ) AS c(next_offset)
    WHERE rcte.[path] IN (0, 1)                             --- We don't have to explore both forwards and backwards from S - one is enough
      AND o.next_offset!=ISNULL(rcte.prev_offset, -1)       --- No backtracking
      AND NOT (rcte.step>0 AND rcte.offset=@S)              --- Stop recursion when we hit the S.
)
...
CREATE UNIQUE CLUSTERED INDEX IX ON #path (step);

--- Count the number of steps, divide by two to get the half-way point:
SELECT MAX(step)/2 FROM #path;
...
--- Create a geometry object from the path above:
DECLARE @geo geometry;  
SELECT @geo=geometry::STGeomFromText('POLYGON(('+
        STRING_AGG(CAST((offset-1)%@width+1 AS varchar(max))+' '+CAST(offset/@width+1 AS varchar(max)), ', ') WITHIN GROUP (ORDER BY step)
        +'))', 0)
FROM #path;

--- Create a "canvas", same width and height as our source dataset,
--- and generate squares for each point...
WITH points AS (
    SELECT geometry::STGeomFromText('POLYGON(('+
                CAST(coords.x-0.5 AS varchar(max))+' '+CAST(coords.y-0.5 AS varchar(max))+', '+
                CAST(coords.x-0.5 AS varchar(max))+' '+CAST(coords.y+0.5 AS varchar(max))+', '+
                CAST(coords.x+0.5 AS varchar(max))+' '+CAST(coords.y+0.5 AS varchar(max))+', '+
                CAST(coords.x+0.5 AS varchar(max))+' '+CAST(coords.y-0.5 AS varchar(max))+', '+
                CAST(coords.x-0.5 AS varchar(max))+' '+CAST(coords.y-0.5 AS varchar(max))+'))', 0) AS geo
    FROM GENERATE_SERIES(1, CAST(LEN(@input)/@width AS int), 1) AS y
    CROSS APPLY GENERATE_SERIES(CAST(1 AS smallint), @width, CAST(1 AS smallint)) AS x
    CROSS APPLY (
        VALUES (y.[value], x.[value])
        ) AS coords(y, x))

--- ... and check which of those squares are located in the @geo polygon:
SELECT COUNT(*)
FROM (VALUES (@geo)) AS g(selection)
CROSS JOIN points
WHERE g.selection.STContains(points.geo)=1;
