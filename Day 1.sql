DECLARE @input varchar(max)='9eightone
...
--- Part 1:
SELECT SUM(CAST(c.ch AS int))
FROM STRING_SPLIT(REPLACE(@input, CHAR(13), ''), CHAR(10)) AS i
CROSS APPLY (
    VALUES (PATINDEX('%[0-9]%', i.[value]),
            LEN(i.[value])+1-PATINDEX('%[0-9]%', REVERSE(i.[value])))
    ) AS o(first_offset, last_offset)
CROSS APPLY (
    VALUES (SUBSTRING(i.[value], o.first_offset, 1)+
            SUBSTRING(i.[value], o.last_offset, 1))
    ) AS c(ch);
...
--- Part 2:
WITH numbers AS (
    SELECT CAST([value] AS varchar(10)) AS string, ss.ordinal AS num
    FROM STRING_SPLIT('one,two,three,four,five,six,seven,eight,nine', ',', 1) AS ss
    UNION ALL
    SELECT '0', 0
    UNION ALL
    SELECT CAST(num AS varchar(10)) AS string, num
    FROM numbers
    WHERE string LIKE '[a-z]%'
)

SELECT SUM(10*first_num.num+last_num.num)
FROM STRING_SPLIT(REPLACE(@input, CHAR(13), ''), CHAR(10)) AS i
CROSS APPLY (
    SELECT TOP (1) num
    FROM numbers
    WHERE i.[value] LIKE '%'+string+'%'
    ORDER BY CHARINDEX(string, i.[value])
    ) AS first_num
CROSS APPLY (
    SELECT TOP (1) num
    FROM numbers
    WHERE i.[value] LIKE '%'+string+'%'
    ORDER BY CHARINDEX(REVERSE(string), REVERSE(i.[value]))
    ) AS last_num;
