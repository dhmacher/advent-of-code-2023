DECLARE @input varchar(max)='....937..........309.............191..............745.................913......................................256................891.......
...
--- a) Split the input into lines and characters:
DROP TABLE IF EXISTS #split;

WITH cte AS (
    SELECT i.ordinal AS [line], n.i AS offset, SUBSTRING(i.[value], n.i, 1) AS s
    FROM STRING_SPLIT(REPLACE(@input, CHAR(13), ''), CHAR(10), 1) AS i
    CROSS APPLY (
        SELECT TOP (LEN(i.[value])) ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS i
        FROM STRING_SPLIT(@input, '.') AS x
        ) AS n)
...
CREATE UNIQUE CLUSTERED INDEX IX ON #split ([line], offset);




--- b) Use the good ol' gaps-and-islands to find whole numbers:
DROP TABLE IF EXISTS #numbers;

SELECT [line],
       MIN(offset) AS from_offset, MAX(offset) AS to_offset, CAST(STRING_AGG(s, '') WITHIN GROUP (ORDER BY offset) AS int) AS num,
       CAST(NULL AS bit) AS is_matched
INTO #numbers
FROM (
    SELECT *, SUM(_new_word) OVER (PARTITION BY [line] ORDER BY offset ROWS UNBOUNDED PRECEDING) AS _word
    FROM (
        SELECT *, (CASE WHEN s='.' THEN 1
                        WHEN s LIKE '[^0-9]' AND LAG(s, 1, '.') OVER (PARTITION BY [line] ORDER BY offset) LIKE '[0-9]' THEN 1
                        WHEN s LIKE '[0-9]' AND LAG(s, 1, '.') OVER (PARTITION BY [line] ORDER BY offset) LIKE '[^0-9]' THEN 1
                        ELSE 0 END) _new_word
        FROM #split
        ) AS sub
    ) AS sub
WHERE s LIKE '[0-9]'
...
CREATE UNIQUE CLUSTERED INDEX IX ON #numbers ([line], from_offset);




--- c) using symbols, look for ajacent numbers; mark those numbers as matched:
UPDATE n
SET n.is_matched=1
FROM #numbers AS n
INNER JOIN (
    SELECT DISTINCT num.[line], num.offset
    FROM #split AS num
    INNER JOIN #split AS sym ON
        num.line BETWEEN sym.line-1 AND sym.line+1 AND
        num.offset BETWEEN sym.offset-1 AND sym.offset+1
    WHERE num.s LIKE '[0-9]'
      AND sym.s LIKE '[^0-9.]'
    ) AS m ON m.[line]=n.[line] AND m.offset BETWEEN n.from_offset AND n.to_offset;



--- d) compute the total for matched numbers:
SELECT SUM(num)
FROM #numbers
WHERE is_matched=1;
...
SELECT SUM(n1.num*n2.num)
FROM #numbers AS n1
INNER JOIN #split AS s ON
        s.line BETWEEN n1.line-1 AND n1.line+1 AND
        s.offset BETWEEN n1.from_offset-1 AND n1.to_offset+1
INNER JOIN #numbers AS n2 ON
        s.line BETWEEN n2.line-1 AND n2.line+1 AND
        s.offset BETWEEN n2.from_offset-1 AND n2.to_offset+1

--- We're only interested in numbers that match a specific symbol:
WHERE n1.is_matched=1
  AND s.s='*'
  AND n2.is_matched=1

--- Avoid multiplying both n1*n2 and n2*n1, which would effectively double the result
  AND (n1.[line]<n2.[line]
       OR
       n1.[line]=n2.[line] AND n1.from_offset<n2.from_offset);
